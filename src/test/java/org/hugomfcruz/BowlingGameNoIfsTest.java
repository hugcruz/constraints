...
public class BowlingGameNoIfsTest extends BowlingGameTests {
    @Before
    public void setUp() {
        bowlingGame = new BowlingGameNoIfs();
    }

    @Test
    public void noIfStatementsAreUsed() throws IOException, ParseException {
        File file = new File("src/main/java/org/hugomfcruz/BowlingGameNoIfs.java");
        CompilationUnit parser = JavaParser.parse(file);
        for (BodyDeclaration member: parser.getTypes().get(0).getMembers()) {
            if(member instanceof MethodDeclaration){
                MethodDeclaration method = (MethodDeclaration) member;

                assertThat(method.getName(), findOperation(method.getBody(), IfStmt.class), is(false));
            }
        }
    }
        ...
        if(body instanceof BlockStmt) {
            for (Statement statement : ((BlockStmt)body).getStmts()) {
                if(statement.getClass().equals(operation)){
                    fail("Forbidden operation used in " + statement);
                }
